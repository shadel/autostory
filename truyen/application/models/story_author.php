 ...
 * @author Shadel
 */
class Story_author extends CI_Model
{
	private $table_name			= 'story_author';			// stories
	private $table_name_story			= 'story_author_story';			// stories
	 ...
	 * @return	object
	 */
	function get_author_by_story_id($story_id)
	{
// 		$this->db->select('*');
...
// 		return NULL;
	...
	function get_author_by_permalink($permalink)
	{
		$this->db->select('*');
		$this->db->from($this->table_name.' as si');
	 ...
	 * @return	array
	 */
	function create_story_author($data)
	{
		if(isset($data['story'])){
			$story = $data['story'];
			unset($data['story']);
		...
		if(!$author_id){
			$author_id = $this->get_author_by_permalink($data['permalink'])->id;
		}
		
		if(isset($story)){
			$sql = $this->db->insert_string($this->table_name_story, array(
					'story' => $story,
					'author' => $author_id)) . ' ON DUPLICATE KEY UPDATE author="'.($author_id).'"';
				
			$this->db->query($sql);
			return array('author_id' => $author_id);
		}
